...
Every script under `scripts/` and every post-processing tool under `postprocess/tools/` must follow both of these — they apply to all future tools, not just the ones that exist today. `postprocess/` itself is organized by pipeline stage (`stage1/` parsing through `stage6/` report assembly), with CLI tools in their own `postprocess/tools/` directory and everything mutually importable via a shared `postprocess/_stage_paths.py` sys.path bootstrap — see that module's docstring.

- **Name the tool after what it does, not the AMD tool it wraps.** The underlying `rocprofv3`/`rocprof-sys`/`rocprof-compute`/`rocgdb` name belongs in code comments and in the `-h`/`--help` text (see below), never in the file name or in the report/output it produces. Established pattern: launcher scripts (`scripts/`) are named `profile_<what>` (e.g. `profile_CPU_hotspots.sh`), post-processing tools (`postprocess/`) are named `extract_<what>` (e.g. `extract_CPU_hotspots.py`), debugging launchers (`scripts/`) are named `debug_<what>` (e.g. `debug_crash.sh`) — follow this pattern for new tools unless there's a good reason not to.
