...
MODULE_PATH = os.path.join(POSTPROCESS_DIR, "stage3", "stage3_rocprofsys_trace.py")

sys.path.insert(0, os.path.abspath(POSTPROCESS_DIR))
import _stage_paths  # noqa: E402  (adds every stageN/tools dir to sys.path)

spec = importlib.util.spec_from_file_location("stage3_rocprofsys_trace", MODULE_PATH)
trace = importlib.util.module_from_spec(spec)
sys.modules["stage3_rocprofsys_trace"] = trace
...
def make_row(name, category, parent=None):
...
class TagRowsTests(unittest.TestCase):
    ...
    def test_numa_category_tagged_other(self):
        row = make_row("numa_migration_event", "numa")
        trace.tag_rows([row])
        self.assertIn("other", row["tags"])

    def test_unmapped_category_tagged_other(self):
        row = make_row("mystery_event", "rocm_totally_new_category_v99")
        trace.tag_rows([row])
        self.assertIn("other", row["tags"])
    ...
    def test_category_tag_and_name_pattern_tag_both_survive(self):
        ...
        row = make_row("gotcha_mpi_helper", "mpi")
        trace.tag_rows([row])
        self.assertIn("mpi_territory", row["tags"])
        self.assertIn("wrapper_noise", row["tags"])
