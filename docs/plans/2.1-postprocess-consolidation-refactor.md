...
> **Divergence (plan 2.12):** this section's design landed as roadmap step 10, largely as
> originally sketched below, with one architectural change decided during planning: the resolved
> `tag_defs` is a **process-wide global** (`stage6_noise_config.py`'s `configure()`/`tag_defs()`),
> not threaded as a parameter through every function between a tool's `main()` and `tag_rows()` —
> every real invocation of this codebase is a one-shot CLI process with exactly one noise-pattern
> configuration for its whole run, so parameter-threading would have been pure signature churn.
> `load_default_patterns()` also moved out of `stage3_rocprofsys.py` into that same new module, so
> the dependency between them is one-directional (stage3 reads from stage6, never the reverse).
> Real-data smoke-testing surfaced one genuine, pre-existing bug this design otherwise wouldn't
> have exposed: pattern matching lowercases a row's own label but never the pattern strings
> themselves, silently relying on `default_noise_patterns.json`'s own patterns already being
> lowercase — a real footgun for a user's own mixed-case `add`/`remove` substrings, now fixed by
> lowercasing them on the way in. See
> [docs/plans/2.12-noise-config-file.md](2.12-noise-config-file.md) for the full as-built design.
