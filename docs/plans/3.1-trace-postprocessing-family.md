...
> **Note (plan 3.5):** roadmap steps 3.5 ("stage4-equivalent: flat + tree") and 3.6 ("on-disk
> aggregate cache") below were merged into one plan, `docs/plans/3.5-trace-aggregate-and-cache.md`
> — the module breakdown's independent flat/tree accumulators and the caching design's "both views
> read the same cache file" claim didn't actually agree with each other; see that plan for the
> corrected design. It also moves `merge_rank_trees()`/`flatten_tree()`/`caller_chains_for_label()`/
> `aggregate_node_stats()`/`make_node_values()` out of `stage4_rocprofsys_sample_tree.py` into a new
> `stage4_rocprofsys_common.py`, mirroring plan 3.4's stage3 split. This file is left as originally
> approved.

> **Note (plan 3.7):** roadmap step 3.7 ("stage5/6 composition") is implemented in
> `docs/plans/3.7-rank-discovery-and-cli-tools.md`, completing this plan's roadmap. Real trace-CSV
> filename evidence (found in the `Heat_Convection_Solver` directory this session's manual
> conversion work produced) resolved the multi-rank file-discovery question this plan deferred; the
> `{tag: action}` wiring needed almost no new design since `stage3_rocprofsys_trace.py` already
> reused the sample pipeline's exact tag names. See that plan for the full accounting.
